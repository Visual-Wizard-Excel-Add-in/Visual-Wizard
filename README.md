...
| **MS Appsource 개인 배포 불가**

</div>

## 🗂️ Index

- [ Visual Wizard](#-visual-wizard)
  - [🔗 Links](#links)
  - [🗂️ Index](#️index)
  ...
  - [🔥 Challenges](#-challenges)
    ...
    - [2. Office JS와 비동기성](#2-office-js와-비동기성)
      - [**2-1. Excel 데이터 조회 및 관리**](#2-1-excel-데이터-조회-및-관리)
      - [**2-2. 비동기 작업의 관리**](#2-2-비동기-작업의-관리)
  - [🚨 Issues](#-issues)
    - [1. 매크로 녹화 기능 구현 이슈](#1-매크로-녹화-기능-구현-이슈)
    - [문제점](#문제점)
    - [해결](#해결)
    - [2. 메일 전송하기 기능 구현 이슈](#2-메일-전송하기-기능-구현-이슈)
    - [문제점](#문제점-1)
    - [해결](#해결-1)
  - [🗓️ Schedule](#️-schedule)
    - [프로젝트 기간: 2024.07.08 ~ 2024.07.31](#프로젝트-기간-20240708--20240731)
  - [⌛️ Memoir](#️-memoir)
...
이런 불편을 해결하고자 Visual Wizard는 입력된 수식에 대한 정보와 관계된 셀들을 쉽게 찾아볼 수 있습니다.
...
- 하단에는 Excel 함수 사전 사이트로 이동할 수 있는 링크가 첨부돼있습니다.
...
- 각 함수명 클릭 시, 함수의 인수, 조건, 결과 값을 보여줍니다.
...
- 녹화 버튼 클릭 후, 버튼을 다시 클릭할 때까지의 **셀 입력, 셀 서식 변경, 차트 추가, 표 추가 액션**을 녹화합니다.
...
- 토글할 경우, 에러가 있는 셀들을 강조합니다.
- 값이 입력 된 가장 마지막 위치의 셀 주소를 표시합니다.
...
- 작성한 Excel문서의 특정 부분을 새로운 통합 문서로 추출할 수 있습니다.
...
Add-in이란 Excel과 같은 Office Application 내에서 다양한 추가기능을 제공하는 Web기반 확장프로그램입니다.

- Excel '홈 탭'의 가장 우측에서 접근 가능

<div align="center">
  <img width="600" src="https://github.com/user-attachments/assets/0f87ad23-7071-43b3-8395-eed92b062181">
</div>

- 클릭 시 Excel 화면 우측에 웹 기반 패널이 열림

<div align="center">
  <img width="600" src="https://github.com/user-attachments/assets/fefc909c-0c24-4eed-9a7f-1c524da1e5f6">
</div>

해당 사이드 패널은 웹 페이지 이므로 JavaScript, HTML, CSS를 통한 개발이 가능합니다.

<br/>

#### **1-2. Excel과 Add-in의 데이터 교류 방법**

Web과 Excel간의 상호작용을 위해 **Office JavaScript API**(Office JS)를 활용해야 했습니다.

해당 Office JS는 성능 최적화를 위해 통합 문서의 모든 내용을 한번에 불러오지 않습니다.
...
이런식으로 Excel과 Add-in과의 데이터 교환은 Add-in에서의 요청 작업을 모아 **context.sync()** 로 요청한 작업들을 수행하고 동기화하는 과정을 통해서 이루어집니다.

데이터 접근은 API상에서 정해진 Worksheet, Range, Chart 등의 객체에 대해서만 접근 할 수 있으며, 모든 작업들은 비동기적으로 이루어집니다.
...
Add-in 개발에 있어서 가장 신경쓴 점은 Office JS에 적응하는 것이었습니다.<br/>
Excel 데이터에 접근하기 위해선 Office JS의 함수와 메서드에 익숙해져야 했으며, 그 모두가 비동기적으로 이루어진다는 환경에 적응해야했습니다.

Excel에는 다양한 종류의 객체들이 존재하며, 해당 객체들의 속성과 지원하는 메서드가 모두 다르기 때문에 하나의 속성을 조회하고 수정하기 위해선, console과 공식 문서를 끊임없이 들여다 봐야 했습니다.

#### **2-1. Excel 데이터 조회 및 관리**

만약 특정 셀의 서식을 저장하고 싶다면 해당 셀의 서식을 지칭하는 속성들의 종류를 조회하고, 해당 속성들이 지원하는 값들을 조사해야했습니다.
...
위와 같이 명시적으로 서식 속성들을 `load()` 한 후 `context.sync()`를 통해 동기화 한 후에야 해당 속성들을 사용할 수 있습니다.

해당 서식 속성들을 효율적으로 관리하며, 사용자가 프리셋 단위로 저장하고 적용 시킬 수 있도록 다음과 같은 자료구조를 구성하였습니다.
...
- **동기화 시점 최적화**: 필요한 시점에만 `context.sync()`를 호출하여 불필요한 동기화 작업을 최소화.
- **의도치 않은 에러 처리**: `try/catch`구문의 사용으로 의도치 않은 에러를 파악하고, 사용자에게 Message Box를 팝업하여 에러 사항을 인지할 수 있도록 적절히 처리하였습니다.
...
초기 의도한대로 Add-in과 VBA로 Excel Application의 매크로 녹화 기능을 작동시키는데 성공하였지만, 사용자가 인터페이스를 직접 클릭하지 않는 이상 VBA 코드 녹화가 진행되지 않는다는 문제점에 봉착하였습니다.

### 해결

정식 매크로 기능을 사용할 수 없으므로, Office JS만을 활용하여 매크로 기능을 구현했습니다.<br/>
...
Office JS의 Outlook API를 동시에 이용하려 했지만, Excel 내에서 Outlook과의 통신이 불가능하다는 것을 알았습니다.

### 해결

1. 기획 변경<br/>
   기획 의도가 메인 타겟인 사무직분들의 특정 부분을 메일로 요청받을 경우의 불편함을 해소하기 위함이었던 만큼, 해당 불편함을 다른 방도로 해소하기 위해 사용자가 희망하는 범위를 새로운 Excel 통합 문서로 추출하는 기능으로 변경했습니다.

   하지만 Office JS로 구현하기엔 다음과 같은 제약 사항이 있었습니다.

   - 모든 속성들을 전부 로드, 저장하여야함.
   - 완벽히 복사, 붙여넣기엔 한정된 API지원 속성들.

   이를 극복하기위해 VBA를 활용했습니다.

> [!NOTE]
> VBA(Visual Basci for Application): MS Office에 내장된 프로그래밍 언어로, Excel에 깊숙이 통합되어있어 다양한 기능들을 보다 자유롭게 구현할 수 있습니다.<br/>
>
> ### 하지만 직접적인 `Add-in`과 `VBA`간의 상호작용은 지원되지 않습니다!

2. Add-in과 VBA의 연계<br/>
...
또한, VBA 함수는 사용자가 직접 등록해야하기 때문에 사용자가 추출하기 기능 이용전에, 사용 방법 및 해당 VBA 추가 기능 등록 파일을 담은 Notion 사이트를 이용할 수 있도록 Message Box를 볼 수 있도록 했습니다.<br/>
...
모든 과정을 혼자서 소화해야하는 개인 프로젝트는 생각했던 것 이상으로 촉박한 일정과 강행군의 연속이었습니다.
Office JS를 주축으로 진행하는 프로젝트인 이상 사용법 파악과 문제 해결은 온전히 저 개인의 몫이었습니다.<br/><br/>
프로젝트를 진행하며 컨셉이나 구현 방향성에 대해 토론하고 의견을 나누는 과정이 없었기에 막상 구현할 때가 되어서 처음 기획 의도와 달라 당황하게 된 경험을 했습니다.<br/>
이번 프로젝트를 바탕으로 기획 단계에서 좀 더 계획 및 구현 방안을 구체화하는 것의 중요성을 깨달았습니다.<br/><br/>
하지만 동시에, 코딩의 코 자도 모르던 3개월 전과는 다르게 저만의 힘으로 이와같은 프로젝트를 완성켰다는 뿌듯함 또한 느낄 수 있었습니다.<br/>
3개월로 이뤄낸 성과가 무색해지지 않도록 노력해나갈 수 있는 원동력을 얻게된 시간이었습니다.
